...
class TxtSQL
{
    ...
    /**
     * Contains a cache of any files that have been read to increase execution
     * time
     *
     * @var array
     * @see    readFile()
    ...
    public function __construct($path = './data')
    {
        if (!is_dir($path)) {
            return $this->_error(
                E_USER_ERROR,
                'Invalid data directory specified'
            );
        }
     ...
     *
     * @param array $arguments The arguments in form of "[$key] => $value"
     *                         where $key can be 'db', 'table', 'select',
     *                         'where', 'limit' and 'orderby'
     *
     * @return mixed $results An array that txtSQL returns that matches the
     *               given criteria
     */
     ...
     *
     * @param array $arguments The arguments in form of "[$key] => $value"
     *                         where $key can be 'db', 'table', 'where',
     *                         'limit', and 'values'
     *
     ...
     * Renames a database
     *
     * @param array $arguments The arguments in form of "[old db name], [new db
     *                         name]"
     *
     * @return
     */
    ...
    public function query($statement)
    {
        ...
        if (!$this->_isConnected()) {
            return $this->_error(
                E_USER_NOTICE,
                'Can only perform queries when connected!'
            );
        }
     ...
     * The $action can be either 'show databases', 'create databases', 'drop
     * database', 'rename database'
     * 'show tables', 'create table', 'drop table', 'alter table', 'describe',
     * 'select', 'insert', 'delete', and 'insert'. See the readme for more
     * information.
    ...
    public function execute($action, $arguments = null)
    {
        ...
        if (empty($action) || !is_string($action)) {
            return $this->_error(
                E_USER_NOTICE,
                'You have an error in your txtSQL query'
            );
        }
        ...
        if (!empty($arguments) && !is_array($arguments)) {
            return $this->_error(
                E_USER_NOTICE,
                'txtSQL Can only accept arguments in an array'
            );
        }
    ...
    public function lockDb($db)
    {
        ...
        if (!$this->_isConnected()) {
            ...
            return $this->_error(
                E_USER_NOTICE,
                'Lock for database \''.$db.'\' already exists'
            );
        }
    ...
    public function unlockDb($db)
    {
        ...
        if (!$this->_isConnected()) {
            ...
            return $this->_error(
                E_USER_NOTICE,
                'Lock for database \''.$db.'\' doesn\'t exist'
            );
        }
     ...
     *
     * @param string $table Table to be checked for existence
     * @param string $db    The database the table is in
     *
     ...
     *
     * @param string $db DB to be checked for existence
     *
     * @return bool Whether it exists or not
     */
     ...
     * @param string $column
     *
     * @return string $column Get the last ID generated by this column instead
     *                of the priamry key (optional)
     */
    public function lastInsertId($table, $db = '', $column = '')
    {
        ...
        if (!empty($column)) {
            if ($this->_getColPos($column, $cols) === false) {
                ...
                return $this->_error(
                    E_USER_NOTICE,
                    'Column \''.$column.'\' is not an auto_increment field'
                );
            }
        ...
        elseif (empty($cols['primary']) && empty($column)) {
            return $this->_error(
                E_USER_NOTICE,
                'There is no primary key defined for table \''.$table.'\''
            );
        }
     ...
     * Checks for a connection, and valid arguments
     *
     * @param array $arguments The arguments to validify
     *
     * @return bool
     */
    private function _validate(array $arguments)
    {
        /* Check to see user is connected */
        if (!$this->_isConnected()) {
            return $this->_error(
     ...
     * @param bool   $useCache    Whether to save/retrieve this file from a
     *                            cache
     * @param bool   $unserialize Whether to unserialize the string or not
     *
     * @return array|bool $contents The file's contents
     */
     ...
     * @return bool $success
     */
    private function _lockedFilewrite(
        $filename,
        $mode,
        $contents = '',
        $timeLimit = 300000,
     ...
     */
    protected function _writeFile($filename, $mode = 'w', $contents = '')
    {
            ...
            if (($fp = @fopen($filename, $mode)) !== false) {
                ...
                if (@flock($fp, LOCK_EX) !== false) {
                    ...
                    if (@fwrite($fp, $contents) !== false) {
                        ...
                        $this->_error(
                            E_USER_ERROR,
                            'Error when writing to file \''.$filename.'\''
                        );
                    }
                } else {
                    $this->_error(
                        E_USER_ERROR,
                        'Error creating file lock for \''.$filename.'\''
                    );
                }
            } else {
                $this->_error(
                    E_USER_ERROR,
                    'Error when opening file \''.$filename.'\' for writing'
                );
            }
        }
     ...
     * @return bool Whether the db exists or not
     */
    private function _tableExist($table, $db)
    {
        ...
        foreach ($where as $key => $value) {
            ...
            if ($key % 2 == 1) {
                ...
                if (($and === false) && ($or === false) && ($xor === false)) {
                    return $this->_error(
                        E_USER_NOTICE,
                        'Only boolean seperators AND, and OR are allowed'
                    );
                }
     ...
     */
    private function _generatePhpIf($clause, array $cols)
    {
        ...
        if (substr_count($column, '(') > 0
            && substr_count($column, '(') == substr_count($column, ')')) {
            ...
            if (empty($function)) {
                ...
                $subParser->parseWhere($column = []);

                /* Generate PHP code for the nested part */
                $code .= $test = $this->_generateWhereClause(
                    $column['where'],
                    $cols
                );
            } else {
                /* Generate PHP code for this part of the clause */
                if (($last = $this->_generateClause($column, $cols))
                    === false) {
                    return false;
                }
                $code .= $last;
        ...
        if (!empty($op)) {
            ...
            if ($op == '!?' || $op == '?') {
                ...
                if (($functionClause = $this->_generateClause($value, $cols))
                    === false) {
                    return false;
                }

                $code .= '( strpos('.$last.', '.$functionClause.') '.($op == '?'
                        ? '!' : '=').'== FALSE )';
    ...
    /**
     * To generate PHP-equivalent for a section of a WHERE clause (either the
     * column or the value)
     *
     * @param string $clause The clause that needs to be evaluated
     * @param array  $cols   The column definitions array
     *
     * @return string $code The php code for the clause
     */
    protected function _generateClause($clause, array $cols)
    {
        ...
        while ($word = $parser->getNextWord(true)) {
            ...
            if (!$this->_isTextString($word) && !is_numeric($word)) {
                ...
                if (substr_count($word, '(') > 0
                    && substr_count($word, '(') == substr_count($word, ')')) {
                    if (($code .= $this->_generateClause($word, $cols))
                        === false) {
                        return false;
                    }
                ...
                elseif (($word == '+') || ($word == '-') || ($word == '/')
                    ...
                    switch (true) {
                        /* Function or column */
                        case (!$this->_isTextString($nextType)
                            && !is_numeric(
                                $nextType
                            )) :
                            {
                    ...
                    if ($word == '+') {
                        if (($lastType == 'col' && $nextType == 'str')
                            || ($lastType == 'str' && $nextType == 'col')
                            || ($lastType == 'str' && $nextType == 'str')
                            || ($lastType == 'col' && $nextType == 'col')
                            || ($lastType == 'func' && $nextType == 'str')
                            || ($lastType == 'str' && $nextType == 'func')) {
                            $word = '.';
                        }
                    ...
                    if (strtolower(trim($word)) == 'primary') {
                        ...
                        if (empty($cols['primary'])) {
                            return $this->_error(
                                E_USER_NOTICE,
                                'No primary key has been assigned to this table'
                            );
                        }
     ...
     */
    protected function _getColPos($colname, array $cols)
    {
     ...
     * @return array|bool
     */
    protected function _uniqueArray(array $array, $sub_key)
    {
        $target = [];
        ...
        return $this->_error(
            E_USER_NOTICE,
            'Column \''.$sub_key
            .'\' is not in result set; cannot determine distinct values'
        );
    }

    /**
     * Checks whether a variable contains a string or not (inside single or
     * double quotes)
     *
     * @return bool TRUE/FALSE
     */
    protected function _isTextString($string)
    {
