...
class TxtSQLCore extends TxtSQL
{
    ...
    /**
     * To extract data from a database, given that the row fits the given
     * credentials
     *
     * @param mixed arg The arguments that are passed to the txtSQL as an
     *                  array.
     *
     * @return mixed selected An array containing the rows that matched the
     *               where clause
     */
    public function select(array $arg)
    {
            ...
            if (!$this->_isTextString($value)) {
                ...
                if ($value == '*' && empty($wildCard)) {
                    ...
                    if ((substr_count($value, '(') > 0)
                        ...
                        if (($colPos = $this->_getColPos($value, $cols))
                            === false) {
                            return $this->_error(
                                E_USER_NOTICE,
                        ...
                        /* Look for an alias */
                        $value = isset($arg['aliases'][$value])
                            ? $arg['aliases'][$value] : $value;
                        $arg['select'][$value] = $colPos;
                    }
                }
            } else {
                /* This is not a column, it is a string literal */
                $value1 = isset($arg['aliases'][$value])
                    ? $arg['aliases'][$value] : $value;
                $arg['select'][$value] = [str_replace('\\', '\\\\', $value1)];
            }
        ...
        foreach ($arg['select'] as $key => $value) {
            ...
            if (!is_array($value)) {
                ...
                if (substr_count($value, '(') > 0
                    ...
                    $key = isset($arg['aliases'][$key])
                        ? "'{$arg['aliases'][$key]}'" : "";
                    //@todo
                    if (($code = $this->generateFunctionClause($value, $cols))
                        === false) {
                        return false;
                    }
        ...
        if (!empty($arg['orderby']) && !empty($selected)
            && count($selected) > 0) {
            $function
                = '
			function sort_m ( $a, $b )
            ...
            foreach ($arg['orderby'] as $key => $value) {
                if (!array_key_exists($key, $selected[0])) {
                    ...
                    return $this->_error(
                        E_USER_NOTICE,
                        'Results can only be sorted \'asc\' (ascending) or \'desc\' (descending)'
                    );
                }
     ...
     *
     * @param array arg The arguments that are passed to the txtSQL as an array.
     *
     * @return bool
     */
    public function insert(array $arg)
    {
        ...
        if (empty($this->dbName)) {
            ...
            return $this->_error(
                E_USER_NOTICE,
                'No values given to enter into database'
            );
        }
...
//        $max = count($rows);
        ...
        foreach ($arg['values'] as $key => $value) {
			 ...
			 * yet been exceeded */
            if ($cols[$key]['type'] == 'int' && ($cols[$key]['max'] > 0)
    ...
    /**
     * Removes (a) row(s) that fit(s) the given credentials from a table. If
     * none are specified, it will empty out the table.
     *
     * @param array arg The arguments that are passed to the txtSQL as an
     *                  array.
     *
     * @return int deleted The number of rows deleted
     */
    public function delete(array $arg)
    {
        /* If the user specifies a different database, then
		 * automatically select it for them */
        if (!empty($arg['db'])) {
    ...
    public function update(array $arg)
    {
        ...
        if (empty($arg['limit'])) {
            $arg['limit']['0'] = count($rows);
        } elseif (!is_array($arg['limit']) || !is_numeric($arg['limit'][0])
            || $arg['limit'][0] <= 0) {
            $arg['limit']['0'] = count($rows);
        }
        ...
        foreach ($arg['values'] as $key1 => $value1) {
            ...
            $function .= "\$rows[\$key][ $value1[0] ] = ( $type ) "
                .$this->_generateClause($arg['values'][$key1][1], $cols).";";
        }
     ...
     */
    public function showtables(array $arg)
    {
        ...
        return !empty($tables) ? $tables : [];
    }

    /**
     * Creates a table inside of a database, with the specified credentials of
     * the column
     *
     * @param array arg The arguments that are passed to the txtSQL as an
     *                  array.
     ...
     * @return bool
     */
    public function createtable(array $arg)
    {
     ...
     * @return bool
     */
    public function droptable(array $arg)
    {
        ...
        if (empty($arg['table'])
            ...
            return $this->_error(
                E_USER_NOTICE,
                'Database name can only contain letters, and numbers'
            );
        }
    ...
    /**
     * Alters a table by working with its columns. You can rename, insert,
     * edit, delete columns. Also allows for manipulation of primary keys.
     *
     * @param array arg The arguments that are passed to the txtSQL as an
     *                  array.
     *
     * @return bool
     */
    public function altertable(array $arg)
    {
        ...
        if (!empty($arg['name'])
            ...
            return $this->_error(
                E_USER_NOTICE,
                'Names can only contain letters, numbers, and underscored'
            );
        } elseif (empty($arg['action'])) {
            return $this->_error(
                E_USER_NOTICE,
                'No action specified in alter table query'
            );
        }
     ...
     *                  array.
     *
     * @return mixed cols An array populated with details on the fields in a
     *               table
     */
    public function describe(array $arg)
    {
     ...
     * @return bool
     */
    public function createdatabase(array $arg)
    {
        /* Make sure that we have a name, and that it's valid */
     ...
     * @return bool
     */
    public function dropdatabase(array $arg)
    {
     ...
     * @return bool
     */
    public function renamedatabase(array $arg)
    {
